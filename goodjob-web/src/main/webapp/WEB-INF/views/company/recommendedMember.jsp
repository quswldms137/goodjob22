...
<main>
	...
	<div id="container">
		<h2 class="h2">스킬매칭 추천 구직자</h2>
		<div class="list-area">
			<div class="list-left">
				<h3 class="member_name">스킬매칭 추천 구직자 이름</h3>
			</div>
			<div class="list-mid">
				<div class="list-mid-top">
					<h3 class="list-">스킬매칭 추천 구직자 대표 이력서 제목</h3>
				</div>
				<div class="list-mid-bot">
					<p class="hope_job">스킬매칭 추천 구직자 희망직무</p>
					<p class="hope_city">스킬매칭 추천 구직자 희망근무지역</p>
					<p class="hope_pay">스킬매칭 추천 구직자 희망연봉</p>
					<p class="sk_name">스킬매칭 추천 구직자 보유스킬</p>
				</div>
			</div>
...
<script type="text/javascript">
	...
	let recruitIList = new Array();
	let resumeIList = new Array();
	let recruit = {};
	...
	function recommendedMember(){
		...
		xhttp.onload = function(){
			const recommendedMemberRecruitInfo = JSON.parse(this.responseText);
			console.log(recommendedMemberRecruitInfo);
			for(int i = 0; i < recommendedMemberRecruitInfo.size(); i++){
				comInterestList(recommendedMemberRecruitInfo.get(i)); // 각각의 채용공고(recruit)
				
			}
			inputdata();
		}
		
		xhttp.open("GET", "http://localhost:8888/api/company/recommendedMemberRecruitInfo?username=" + username);
		xhttp.send();
	}
	// resume.sk_name.map(sk => sk).join(', ')
	function comInterestList(data){
		const username = localStorage.getItem("username");
		const xhttp = new XMLHttpRequest();
		
		xhttp.onload = function(){
			const recommendedMemberResumeInfo = JSON.parse(this.responseText);
			console.log(recommendedMemberResumeInfo);
			
			for(int i = 0; i < recommendedMemberResumeInfo.size(); i++){ // 각각의 대표 이력서(resume)
				for(int j = 0; j < recommendedMemberResumeInfo.get(i).skill.size(); j++){
					for(int k = 0; k < ; k++){
						if(data.skill.get(k) == recommendedMemberResumeInfo.get(i).skill.get(j)){
							resumeIList.add(i);
		...
		xhttp.open("GET", "http://localhost:8888/api/company/recommendedMemberResumeInfo");
		xhttp.send();
	}
	...
	function inputdata(data){
		...
		let str = '<h2 class="h2">스킬매칭 추천 구직자</h2>';
		data.forEach(resume =>{
